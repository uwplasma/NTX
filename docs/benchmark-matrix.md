...
| Owned finite-beta Redl and `NTX+NEOPAX` bootstrap-current stress | same finite-beta VMEC wout, Boozer transform, analytic profile contract, production radial/collisionality ladder, adaptive physical `nu/v` support, `D33_spitzer` audit branch, Sonine-order convergence sidecar, coefficient/profile localization sidecar, profile-current observable sidecar, current-conditioning sidecar, closure-quadrature sidecar, and production SFINCS-JAX coefficient ladder sidecar are artifact-backed; the current production coefficient ladder is still looser than the cancellation-conditioned precision needed for a `1e-1` net-current gate, and the only apparent current-gate pass is under-integrated in velocity quadrature, so profile-current closure diagnostics remain open before any finite-beta parity claim |
