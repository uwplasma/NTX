...
- The prepared derivative path reaches a best observed speedup of `3.861x` on the benchmarked electric-field scan.
...
- The implicit fixed-boundary `vmec_jax -> booz_xform_jax -> NTX` diagnostic is mixed on the committed QA case: the equilibrium-volume derivative matches centered finite differences with relative mismatch `9.236e-05`, while the Boozer scalar and NTX transport observables remain open at `7.337e-01` and `6.566e+00`.
- The matching reverse-mode Boozer-scalar diagnostic on the implicit-equilibrium lane remains unavailable because the current JAX transform rejects the implicit dynamic-loop solve on that path.
...
- The artifact-backed geometry-family breadth summary now covers `7` active analytic, file-backed, boundary-projected, explicit-relaxed, and implicit-volume stress cases with maximum active mismatch `3.088e-04`. The remaining open implicit Boozer and NTX transport objectives reach maximum mismatch `6.566e+00` and are excluded from promoted geometry-family claims.
