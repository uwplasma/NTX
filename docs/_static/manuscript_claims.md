...
- The owned finite-beta SFINCS-JAX generation lane now has `6` completed same-grid transport-matrix output(s) and `6` coefficient-level NTX comparison(s). The current maximum same-grid `L13/L31/L33` relative difference is `1.407e-02`; this is a transport-coefficient stress diagnostic, not yet a profile-current parity claim.
