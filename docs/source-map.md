...
| Dense block solve and scans | `src/ntx/solver.py`, `src/ntx/_solver_types.py`, `src/ntx/_solver_context.py`, `src/ntx/_solver_core.py`, `src/ntx/_solver_prepared.py`, `src/ntx/_solver_adjoint.py`, `src/ntx/_solver_derivative_audit.py`, `src/ntx/_solver_factorization.py`, `src/ntx/_solver_scan.py` | `MonoenergeticCase`, `TransportResult`, shared operator-context construction, solve orchestration, SOLVAX adapters, prepared solve wrappers, custom-VJP adjoint algebra, and independently gated derivative audits |
