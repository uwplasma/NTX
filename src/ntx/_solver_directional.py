"""Custom differentiation rules for directional derivatives in Er and nu.

Contributed alongside the NEOPAX coupling work. These sit above the prepared
solve in _solver: they reuse its factorization and adjoint machinery to obtain
directional derivatives without differentiating the elimination itself, so the
module depends on _solver and nothing there depends back on it.

Kept separate from _solver because it is a second concern, and a large one --
the ownership rule in tests/test_source_map.py is what surfaced that.
"""
...
from .operators import parameter_derivative_blocks


def _zero_first_row(block: Array) -> Array:
    """Zero the first row of an operator block.

    Matches the nullspace condition applied to the primal: that row is a
    constraint rather than a physical equation, so it carries no sensitivity.

    (Docstring added while rebasing onto the consolidated modules; correct it
    if it misstates the intent.)
    """
...
def _parameter_gradient_directional_from_adjoint(
    ...
    """Directional counterpart of the parameter gradient contraction.

    Where _solver._parameter_gradient_from_adjoint contracts the adjoint with
    the operator derivative to give d/d(nu, epsi), this also carries the
    directional derivative of the adjoint, so one pass yields both the gradient
    and its derivative along the requested direction.
    ...
    if case.er_hat is not None:
        ...
        return (
            ...
            MonoenergeticCase(
                nu_hat=nu_bar_dot,
                epsi_hat=None,
                er_hat=epsi_bar_dot / transport_scale,
            ),
        )
    return (
...
def solve_prepared_coefficient_vector_lowdot_two_pullbacks(
    ...
    def _two_direction_pullbacks(
        ...
        def _contract_direction_source_bar_pair_scan(
            source_dot_pair_for_mode, source_bar_pair_for_mode
        ):
            direction_count = coefficient_bar_pair.shape[0]
            mode_count = saved_lu.shape[0]
            last_index = mode_count - 1
        ...
        def source1_dot_pair_for_mode(k):
            return _source_dot_pair_for_mode(k)[0]
        def source3_dot_pair_for_mode(k):
            return _source_dot_pair_for_mode(k)[1]
    ...
    def _scan_direction_pullbacks(
        ...
        def _one_direction_pullback(xs):
            ...
            def _accumulate_base_bars(carry, k):
                ...
                nu_bar = (
                    nu_bar
                    - lambda1_k @ (diagonal_nu @ f1_k)
                    - lambda3_k @ (diagonal_nu @ f3_k)
                )
            ...
            def source1_dot_for_mode(k):
                return _source_dot_pair_for_direction(k, nu_hat_dot, epsi_hat_dot)[0]
            def source3_dot_for_mode(k):
                return _source_dot_pair_for_direction(k, nu_hat_dot, epsi_hat_dot)[1]
